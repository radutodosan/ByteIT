...
if ($password != $confirm_password) { // Check if password and confirm password are different
    $can_update = 0;
}
...
if ($can_update == 1) {
    $update_query = "UPDATE ".$table_name." SET fullname = ?, password = ?, email = ? WHERE username = ?";
    
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param("ssss", $name, $password, $email, $username);
